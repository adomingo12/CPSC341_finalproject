...
from src.mypl_semantic_checker import *
from src.mypl_symbol_table import *
...
# POSITIVE TESTS
# test 1
def test_multiple_type_param_access_for_int():
    in_stream = FileWrapper(io.StringIO(
        'int f(int x, string y) {return x + stoi(y);} \n'
        'void main() { } \n'
    ))
    ASTParser(Lexer(in_stream)).parse().accept(SemanticChecker())
   
# test 2 
def test_struct_assignment_to_null():
    in_stream = FileWrapper(io.StringIO(
        'struct S {int x;} \n'
        'void main() { \n'
        '  S s = null; \n'
    ...
    ASTParser(Lexer(in_stream)).parse().accept(SemanticChecker())
# test 3
def test_struct_rvalue_assignment():
    in_stream = FileWrapper(io.StringIO(
...
# test 4
def test_new_rvalue_assignment():
    in_stream = FileWrapper(io.StringIO(
...
# NEGATIVE TESTS  
# test 6
def test_bad_rvalue_array_path_type(): 
    in_stream = FileWrapper(io.StringIO(
        'struct S1 {double val; array S1 s;} \n'
        'void main() { \n'
        '  S1 p = new S1(null, null); \n'
...
# test 7
def test_bad_rvalue_path_assignment(): 
    in_stream = FileWrapper(io.StringIO(
        'struct S1 {double val; S1 s;} \n'
        'void main() { \n'
        '  S1 p = new S1(null, null); \n'
...
# test 8
def test_new_rvalue_undefined_struct():
    in_stream = FileWrapper(io.StringIO(
        'void main() { \n'
        '  Point p = new Point(); \n'
        '} \n'
...
def test_expr_with_parens_and_vars():
    in_stream = FileWrapper(io.StringIO(
        ...
        '  int y = (5 - 6) - 7; \n'
        '  int z = ((x / y) + x - y) / (x + y); \n' 
        '  double a = (1.0 + 2.1) + (3.3 * 4.4); \n'
        '  double b = (5.5 - 6.6) - 7.7; \n'
        '  double c = ((a / b) + b - a) / (a + b); \n'
        '  bool d = not (true or false); \n'
        '  bool e = true and not d; \n'
        '  bool f = (d and e) or (not d and e) or (d and not e); \n'
        '  string g = "a" + "b"; \n'
        '  string h = (g + "c") + ("c" + g); \n'
        '  bool i = x or z and d; \n'
        '} \n'
...
# test 10
def test_bad_assignment_incompatible_types():
    in_stream = FileWrapper(io.StringIO(
        'void main() { \n'
        '  int x = 5; \n'
        '  string y = x; \n'  # Attempting to assign an int to a string variable
        '} \n'
    ))
    with pytest.raises(MyPLError) as e:
        ASTParser(Lexer(in_stream)).parse().accept(SemanticChecker())
